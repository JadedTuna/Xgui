...
namespace Xgui
{
    ...
    {
    public:
        Input(Widget *parent, int x, int y, size_t w, size_t h, Flags flags = Flags::NoFlags);
        Input(Widget *parent, const Point &pos, const Size &size, Flags flags = Flags::NoFlags);
        Input(Widget *parent, const Rect &rect, Flags flags = Flags::NoFlags);
        ~Input(void);
