...
namespace Xgui
{
        ...
        , _doubleBuffered(widget->doubleBuffered())
    {
        if (_doubleBuffered) {
            ...
            _drawable = XdbeAllocateBackBufferName(Xgui::display, _widget->X11Window(), XdbeBackground);
        }

        _xftdraw = XftDrawCreate(Xgui::display, _drawable,
            Xgui::screen->XScreen()->root_visual,
            Xgui::screen->XScreen()->cmap);

        XRenderPictFormat *pictformat = XRenderFindVisualFormat(Xgui::display,
                                                                DefaultVisual(Xgui::display,
                                                                              DefaultScreen(Xgui::display)));
        assert(pictformat);
        XRenderPictureAttributes attrs;
        attrs.repeat = 1;
        _xrSolidPicture = XRenderCreatePicture(Xgui::display, _drawable, pictformat, CPRepeat, &attrs);
    ...
    Painter::~Painter(void)
    {
        ...
        if (_doubleBuffered) {
            ...
            if (Xgui::xdbeEnabled()) {
                ...
                assert(XdbeDeallocateBackBufferName(Xgui::display, _drawable));
            }
        }
        XRenderFreePicture(Xgui::display, _xrSolidPicture);
    ...
    void Painter::setTextColor(const Color &color)
    {
        ...
        XftColorAllocValue(Xgui::display,
            Xgui::screen->XScreen()->root_visual,
            Xgui::screen->XScreen()->cmap,
            &xrcolor, &__textColor);
        _textColor = color;
    ...
    void Painter::swapBuffers(void)
    {
        if (_doubleBuffered) {
            if (Xgui::xdbeEnabled()) {
                ...
                assert(XdbeSwapBuffers(Xgui::display, &swapInfo, 1));
                //XFlush(Xgui::display);
            }
        }
    }
    ...
    void Painter::setClip(const Rect &rect)
    {
        ...
        XRenderSetPictureClipRegion(Xgui::display, _xrSolidPicture, r);
        XDestroyRegion(r);
    }

    void Painter::fillRect(int x, int y, size_t w, size_t h)
    {
        XRenderFillRectangle(Xgui::display, PictOpSrc, _xrSolidPicture, &__color, x, y, w, h);
    }
